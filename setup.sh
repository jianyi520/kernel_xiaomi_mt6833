...
setup_kernelsu() {
    ...
    if [ ! -d "$KERNEL_ROOT/KernelSU" ]; then
        proxychains git clone https://github.com/SukiSU-Ultra/SukiSU-Ultra KernelSU
        echo "[+] Repository cloned."
    fi
    cd "$KERNEL_ROOT/KernelSU"
    proxychains git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout main && echo "[-] Switched to main branch."
    fi
    proxychains git pull && echo "[+] Repository updated."
    if [ -z "${1-}" ]; then
